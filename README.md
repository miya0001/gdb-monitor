...
| NGSI-LD エンティティ API | エンティティの一覧取得、タイプ一覧の取得（`/types`）、システム属性（`sysAttrs`）、offset ベースのページネーション | `app.js` |
...
| SDK（npm パッケージ） | `@geolonia/geonicdb-sdk` として npm からインストール | `main.js`, `auth.js` |
...
4. 左側のサイドバーをクリック（またはキーボード操作）するとエンティティにフォーカス
5. サイドバーを下にスクロールすると、次のページが自動的に読み込まれ、地図にも追加表示される（無限スクロール）
6. WebSocket 接続中はリアルタイムでエンティティの追加・更新が反映
7. 時系列データがある場合はスパークラインチャートで自動表示
8. 地図を回転させるとコンパスボタンが表示され、タップで北向きに戻る
